...
const CONCURRENCY = 8
const CHUNKS = ["gctu-chunk-A.json", "gctu-chunk-B.json", "gctu-chunk-C.json", "gctu-chunk-D.json"]
...
async function seedOne(p, userByGhost) {
  ...
  const rows = (p.comments || []).map(c => {
    ...
    return { postId: post.id, userId: commenter.id, ghostId: commenter.ghostId, text: c.text, createdAt: at > new Date() ? new Date() : at }
  })
...
async function main() {
  console.log("Seeding GCTU MEGA feed ...")
  const all = []
  for (const f of CHUNKS) all.push(...JSON.parse(readFileSync(join(process.cwd(), f), "utf-8")))
  ...
  for (let i = 0; i < entries.length; i += BATCH) {
    await Promise.all(entries.slice(i, i + BATCH).map(async ([ghostId, avatarEmoji]) => {
  ...
  for (let i = 0; i < all.length; i += CONCURRENCY) {
    ...
    if (done % 100 < CONCURRENCY) console.log(`PROGRESS posts=${done} comments=${cc}`)
  }
