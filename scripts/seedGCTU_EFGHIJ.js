...
async function main() {
  ...
  for (const f of FILES) {
    const arr = JSON.parse(readFileSync(join(process.cwd(), f), "utf-8"))
    console.log(`${f} TOTAL=${arr.length}`)
    all.push(...arr.map(p => ({...p, _file: f})))
  }
  console.log("GRAND_TOTAL=" + all.length)
  ...
  console.log("GHOSTS=" + ghostMap.size)
  const entries = [...ghostMap.entries()]
  for (let i=0;i<entries.length;i+=20) {
    await Promise.all(entries.slice(i,i+20).map(async ([ghostId, avatarEmoji]) => {
    ...
    console.log(`USERS ${Math.min(i+20,entries.length)}/${entries.length}`)
    await sleep(300)
  }
  ...
  for (const p of all) {
    try {
      const r = await seedOne(p, userByGhost)
      done += r.posts; cc += r.comments
      if (done % 25 === 0) console.log(`PROGRESS posts=${done}/${all.length} comments=${cc}`)
      await sleep(80)
    } catch(e) { console.log(`SKIP post ${p.authorGhost} err=${e.code||e.message}`) }
  }
  console.log(`DONE posts=${done} comments=${cc}`)
}
